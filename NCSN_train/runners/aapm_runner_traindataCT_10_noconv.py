...
class trainset_loader(Dataset):
    def __init__(self):
        ...
        """
        初始化训练集加载器，读取指定目录下所有.npy文件路径。
        """
        # Load .npy files from fixed data/image_domain directory
        # data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data', 'image_domain')
        data_dir = "/home/arnie/Project/DCDS-Dual-domain_Collaborative_Diffusion_Sampling/data/image_domain"
...
class testset_loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return data_array_10
        """
        根据索引加载对应.npy文件，归一化处理，
        并扩展为10通道，转换为(C, H, W)格式返回。
        """
        file_A = self.files_A[index]
        # Load image from .npy
